<<<<<<< Updated upstream:DATA 621 HW5_draft1 (1).Rmd
---
title: DATA621 HW 5
...
```{r}
trainIndex <-  sample(1:nrow(imputed_df), round(0.7*nrow(imputed_df)), replace = FALSE) 
trainData <- imputed_df[trainIndex, ]
testData <- imputed_df[-trainIndex , ] 
```
## Model Building
...
One rule of thumb is to choose the model with the fewest features where the BIC (or other selected metric) is within one SD of the mean. By this measure, the model with only 1 feature, **STARS** is the best one, while if we use adjusted R^2, which penalizes additional features less, the model wiht 3 features (**LableAppeal**, **AcidIndex**, and **STARS**) performs the best. In order to make a final choice, we should see which model performst the best in cross-validaiton.

```{r}
model_final <- lm(TARGET ~ LabelAppeal + AcidIndex + STARS, trainData)
plot(model_final)
```
##Conclusions
...
>>>>>>> Stashed changes:DATA 621 HW5.Rmd
